...
<h1 className="text-one text-3xl font-extrabold" >Choose the Exam</h1>
      {/* Category */}
    {/* Category */}
...
<div data-aos="fade-up-right">
  ...
  >
    <option className="text-sm" value="">Select Course</option>
...
{/* Exam Code */}
<div data-aos="fade-up-right">
  <label className="block mb-1 text-gray-600 font-medium">Exam Code</label>
  <select
    value={selectedExam}
    onChange={(e) => setSelectedExam(e.target.value)}
    className="w-full py-3 px-4 text-one bg-white border-2 border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-one focus:border-one transition-colors duration-300 custom-select"
  >
    <option className="text-sm" value="">Select Exam Code</option>
...
<div data-aos="fade-down-right">
  ...
  <select
    ...
    className="w-full py-3 px-4 text-one bg-white border-2 border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-one focus:border-one transition-colors duration-300 custom-select"
  >
    <option className="text-sm" value="">Select Year</option>
    {years.map((year) => (
...
{/* Month */}
<div data-aos="fade-down-right">
  <label className="block mb-1 text-gray-600 font-medium">Month</label>
  <select
    value={selectedMonth}
    onChange={(e) => setSelectedMonth(e.target.value)}
    className="w-full py-3 px-4 text-one bg-white border-2 border-gray-300 rounded-xl shadow-sm focus:outline-none focus:ring-2 focus:ring-one focus:border-one transition-colors duration-300 custom-select"
  >
    <option className="text-sm" value="">Select Month</option>
    {months.map((month) => (
      <option className="text-sm" key={month.value} value={month.value}>
        {month.name}
      </option>
    ))}
  </select>
...
const filteredPayload = Object.fromEntries(
  Object.entries(payload).filter(([key, value]) => value !== "" && value !== null && value !== undefined)
    ...
    axios.post("https://login.mathshouse.net/api/exam/filter", filteredPayload)
      .then((response) => {
...
>
 {loading ? "Loading..." : "Confirm Selection"}
  ...
  <div className="flex-1 flex justify-center items-center p-4 " data-aos="flip-up">
    <img
      src={think} 
      alt="Illustration"
...
<div ref={sectionRef} className="">
 {dataex && dataex.length > 0 &&(
     <h2 className="text-center text-one text-2xl py-4 font-medium">Exams</h2>
  ...
  data-aos="flip-up"
    key={exam.id}
    href={`https://login.mathshouse.net/login/exams/${exam.id}`}
